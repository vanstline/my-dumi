...
// ─── 默认 Indicator（Auron logo + 旋转弧） ───

function DefaultIndicator() {
  return (
    <span className="auron-loading-indicator">
      <svg
...
export type LoadingProps = AntSpinProps & {
  /** 是否为嵌套区域的遮罩加 glass 效果（仅 wrapper 模式生效） */
  glass?: boolean;
};

/**
 * Auron 加载指示器
 * 基于 antd v4 Spin，支持自定义 SVG indicator、三种尺寸与 glass 遮罩
 */
const Loading: React.FC<LoadingProps> = ({
  ...
  const cls = `auron-spin ${className}`.trim();
  const wrapperCls = glass
    ? `auron-spin-wrapper--glass ${wrapperClassName}`.trim()
    : wrapperClassName;
